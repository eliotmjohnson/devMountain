...
const changeOnMouseover = (evt) => {
	evt.target.style.color = "white";
	evt.target.style.backgroundColor = "black";
};

sayHelloButton.addEventListener("mouseover", changeOnMouseover);
...
const changeOnMouseout = (evt) => {
	evt.target.style.color = "black";
	evt.target.style.backgroundColor = "#EFEFEF";
};

sayHelloButton.addEventListener("mouseout", changeOnMouseout);
...
const repeatMyParam = (evt) => {
	//YOUR CODE HERE
	axios
		.get(`http://localhost:3000/repeat/eliot`)
		.then((res) => {
...
const query = (evt) => {
	axios.get(`http://localhost:3000/query-test?eliot=lol`).then((res) => {
		console.log(res.data);
	});
};

let qButton = document.querySelector("#query-button");
qButton.addEventListener("click", query);
...
/*
    ...
    just sends both queries at the same time
*/
...
const createFood = (evt) => {
	...
	axios.post("http://localhost:3000/food", body).then((res) => {
		...
		for (let i = 0; i < res.data.length; i++) {
			...
			newListItem.textContent = res.data[i];
