...
const cityFilter = cities.filter((elem) => {
  if (elem.endsWith("i") === true) return elem;
});
...
//   } else if (elem["color"] !== "orange" || elem["color"] !== "Red") {
...
const totalRice = foods
  .filter((elem) => elem["tags"].includes("rice") === true)
  .reduce((acc, objPrice) => acc + objPrice["price"], 0);

// THE TOTAL

// console.log(`The total rice foods cost is ${totalRice}`);
